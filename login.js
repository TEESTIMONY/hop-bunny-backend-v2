async function login(email, password) {
    ...
    try {
        ...
        if (rememberMe.checked) {
            localStorage.setItem('token', data.idToken); // Changed from data.token to data.idToken
            localStorage.setItem('refreshToken', data.refreshToken); // Adding refreshToken storage
            localStorage.setItem('userId', data.userId);
            localStorage.setItem('username', data.username);
            localStorage.setItem('highScore', data.highScore || 0);
        } else {
            sessionStorage.setItem('token', data.idToken); // Changed from data.token to data.idToken
            sessionStorage.setItem('refreshToken', data.refreshToken); // Adding refreshToken storage
